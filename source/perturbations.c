...
int perturbations_indices(
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_source_TempMultipoles_g == _TRUE_) {
        ppt->has_source_delta_g = _TRUE_;
        ppt->has_source_theta_g = _TRUE_;
      }
      ...
      class_define_index(ppt->index_tp_shear_g,    ppt->has_source_TempMultipoles_g,   index_type,1);
      class_define_index(ppt->index_tp_multT_g,    ppt->has_source_TempMultipoles_g,   index_type,ppr->l_max_g-2);
      class_define_index(ppt->index_tp_multP_g,    ppt->has_source_PolMultipoles_g,    index_type,ppr->l_max_pol_g+1);
  ...
  if (_scalars_) {
    ...
    if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
      ...
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off) {
        ...
        if ( ppt->has_source_TempMultipoles_g == _FALSE_ ) {
        ...
        if ( ppt->has_source_PolMultipoles_g == _FALSE_ ) {
...
int perturbations_sources(
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_TempMultipoles_g == _TRUE_) {
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off && ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
        _set_source_(ppt->index_tp_shear_g) = y[ppw->pv->index_pt_shear_g];
        for (index_tp = 0; index_tp <= ppr->l_max_g-2; index_tp++){
          _set_source_(ppt->index_tp_multT_g+index_tp) = y[ppw->pv->index_pt_l3_g+index_tp];
        }
      } else {
        if (ppw->approx[ppw->index_ap_tca] == (int)tca_on) {
          _set_source_(ppt->index_tp_shear_g) = ppw->tca_shear_g;
        }
        else{
          _set_source_(ppt->index_tp_shear_g) = 0.;
        }
        for (index_tp = 0; index_tp <= ppr->l_max_g-2; index_tp++){
          _set_source_(ppt->index_tp_multT_g+index_tp) = 0.;
        }
      }
    }

    if (ppt->has_source_PolMultipoles_g == _TRUE_) {
      if (ppw->approx[ppw->index_ap_tca] == (int)tca_off && ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
        _set_source_(ppt->index_tp_multP_g) = y[ppw->pv->index_pt_pol0_g];
        _set_source_(ppt->index_tp_multP_g+1) = y[ppw->pv->index_pt_pol1_g];
        _set_source_(ppt->index_tp_multP_g+2) = y[ppw->pv->index_pt_pol2_g];
        _set_source_(ppt->index_tp_multP_g+3) = y[ppw->pv->index_pt_pol3_g];
        for (index_tp = 4; index_tp <= ppr->l_max_pol_g; index_tp++){
          _set_source_(ppt->index_tp_multP_g+index_tp) = y[ppw->pv->index_pt_pol3_g+index_tp];
        }
      } else {
        if (ppw->approx[ppw->index_ap_tca] == (int)tca_on) {
          _set_source_(ppt->index_tp_multP_g) = 2.5*ppw->tca_shear_g;
          _set_source_(ppt->index_tp_multP_g+1) = k/ppw->pvecthermo[pth->index_th_dkappa]*(5.-2.*ppw->s_l[2])/6.*ppw->tca_shear_g;
          _set_source_(ppt->index_tp_multP_g+2) = 0.5*ppw->tca_shear_g;
          _set_source_(ppt->index_tp_multP_g+3) = k/ppw->pvecthermo[pth->index_th_dkappa]*3.*ppw->s_l[3]/14.*ppw->tca_shear_g;
        }
        else{
          _set_source_(ppt->index_tp_multP_g) = 0.;
          _set_source_(ppt->index_tp_multP_g+1) = 0.;
          _set_source_(ppt->index_tp_multP_g+2) = 0.;
          _set_source_(ppt->index_tp_multP_g+3) = 0.;
        }
        for (index_tp = 4; index_tp <= ppr->l_max_pol_g; index_tp++){
          _set_source_(ppt->index_tp_multP_g+index_tp) = 0.;
        }
      }
    }
  }
