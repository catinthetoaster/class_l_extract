...
#                raise CosmoSevereError(self.fo.error_message)
    ...
    def get_sources(self):
        ...
        if self.pt.has_source_psi:
            ...
            names.append("psi")
        if self.pt.has_source_TempMultipoles_g:
            print("Has temp multipoles g")
            indices.append(self.pt.index_tp_shear_g)
            names.append("shear_g")
            for i in range(self.pr.l_max_g-2):
              indices.append(self.pt.index_tp_multT_g+i)
              names.append("mult_Tg_{}".format(i+3))
        if self.pt.has_source_PolMultipoles_g:
            for i in range(self.pr.l_max_pol_g+1):
              indices.append(self.pt.index_tp_multP_g+i)
              names.append("mult_Pg_{}".format(i))
        ...
        for index_type, name in zip(indices, names):
            ...
            sources[name] = np.asarray(tmparray).copy()
